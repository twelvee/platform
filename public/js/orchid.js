  ...
  _createClass(_default, [{
    ...
    value: function initDropZone() {
      ...
      this.dropZone = new dropzone__WEBPACK_IMPORTED_MODULE_1___default.a(dropname, {
        ...
        init: function init() {
          ...
          this.on('completemultiple', function () {//  $(`${dropname}.sortable-dropzone`).sortable('enable');
          });
          var images = data;
          ...
          if (images) {
            Object.values(images).forEach(function (item) {
              ...
              _this2.emit('thumbnail', file, file.url);
          ...
          this.on('removedfile', function (file) {
            ...
            axios["delete"](platform.prefix("/systems/files/".concat(file.data.id)), {
    ...
    value: function loadMedia() {
      ...
      axios.post(platform.prefix('/systems/media'), {
        ...
        _this3.mediaList = response.data;
        $("".concat(_this3.dropname, " .media.modal")).modal('show');
...
function (_Controller) {
  ...
  _createClass(_default, [{
    ...
    value: function buildGetUrlParams(paramsObj) {
      var builtUrl = "".concat(window.location.origin + window.location.pathname, "?");
      Object.keys(paramsObj).forEach(function (key) {
        builtUrl += "".concat(key, "=").concat(paramsObj[key], "&");
      });
